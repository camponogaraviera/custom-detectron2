...
def _compress_video_to_size(
    ...
    The frame geometry is preserved exactly. Only codec quality/bitrate is
    adjusted to reduce the final file size.
    """
...
def _get_resized_dimensions(
    ...
    Returns:
        The resized `(width, height)` while preserving aspect ratio.

    Raises:
        ValueError: If `res_factor` is outside `[0, 100]` or would produce a
            zero-height output.
    """
...
def _load_runtime_config(config_path: str) -> dict[str, Any]:
    """Load the dataset settings required by inference.

    Args:
        config_path: Absolute path to the configuration file.

    Returns:
        A dictionary containing class names, class count, and dataset path.
    """
...
class Detectron:
    ...
    Args:
        ...
        model_weights: Path to custom model weights. If omitted, pretrained
            model-zoo weights are used.
    ...
    def _get_default_config(self, device: str) -> CfgNode:
        """Create the base Detectron2 configuration.

        Args:
            device: Inference device, such as `cpu` or `cuda`.

        Returns:
            A Detectron2 configuration object initialized with the device.
        """
        cfg: CfgNode = get_cfg()
...
def main(argv: list[str] | None = None) -> None:
    ...
    runtime_config = _load_runtime_config(CONFIG_FILE_PATH)
    detectron = Detectron(
        model_type=args.model,
        model_weights=args.weights,
        threshold=args.threshold,
        class_names=runtime_config["class_names"],
        n_classes=runtime_config["n_classes"],
